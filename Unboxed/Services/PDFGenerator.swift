...
class PDFGenerator {
    ...
    // Legacy single-page function (deprecated, kept for compatibility)
    private static func createPage(for email: Email, pageNumber: Int, totalPages: Int, settings: AppSettings) async -> PDFPage? {
        ...
        return await withTaskGroup(of: PDFPage?.self) { group in
            group.addTask {
                // Create an NSAttributedString for the content on the main actor
                let content = await MainActor.run {
                    createAttributedContent(for: email, pageNumber: pageNumber, totalPages: totalPages, settings: settings)
                }
                ...
                return await MainActor.run {
                    autoreleasepool {
                        // Create a custom view to render the entire content (full range)
    ...
    @MainActor
    private static func createAttributedContent(for email: Email, pageNumber: Int, totalPages: Int, settings: AppSettings) -> NSAttributedString {
    ...
    @MainActor
    private static func isHTMLContent(_ text: String) -> Bool {
    ...
    @MainActor
    private static func createHTMLAttributedString(from html: String) -> NSAttributedString? {
